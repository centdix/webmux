...
function notify(args, payload) {
  // Detached so the report still lands when opencode exits right after the event that
  // triggered it — the turn-end report is the one webmux notifies on.
...
export const WebmuxBridge = async () => {
  ...
  notify(["status-changed", "--lifecycle", "idle", "--best-effort"])
  ...
  return {
    event: async ({ event }) => {
      ...
      // Only "busy" is forwarded: the idle side of session.status fires alongside
      // session.idle, and racing the two would drop the stopped state.
      if (event.type === "session.status") {
        if (props.status && props.status.type === "busy") {
          notify(["status-changed", "--lifecycle", "running", "--best-effort"])
        }
        return
      }
